...
void prendre_fourchette(int numero){
	...
	if( interblocage){
		//Si il ya interblocage, on le signale
		led_on();
		while(1);	
...
start_kernel ( void )
{
	...
	/* Pour améliorer la visibilité du clignotement lorsqu'un philosophe mange*/
	mtx_init(&mutex_led);	
	/* On ajoute un processus indépendant de philosophes */
